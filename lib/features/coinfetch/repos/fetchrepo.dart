...
class fetchtrpo {
  ...
  Future<BigDataModel> getcoinsprice() async {
    ...
    bigDataModel.dataModel.sort(
        (a, b) => (a.quoteModel.usdModel.price).toDouble().compareTo(b.id));
    // return BigDataModel.fromJson(response.data);
    return bigDataModel;
  }
  ...
  Future<BigDataModel> getcoinsvolumne() async {
    ...
    bigDataModel.dataModel
        .sort((a, b) => a.quoteModel.usdModel.volume24h.compareTo(b.id));
    // return BigDataModel.fromJson(response.data);
    return bigDataModel;
  }

  Future<String> geticon(String name) async {
    dio.options.headers['X-CMC_PRO_API_KEY'] =
        "3316f59c-2878-4884-8d02-b41197093373";
    var response = await dio.get(
        "https://pro-api.coinmarketcap.com/v2/cryptocurrency/info?symbol=$name");
    var s = response.data['data'][name][0]['logo'];
    return s;
  }
}
